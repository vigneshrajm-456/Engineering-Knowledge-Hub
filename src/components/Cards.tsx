  ...
  return (
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <h3 className="text-lg font-bold tracking-tight text-slate-900 dark:text-slate-50 mb-2 line-clamp-2 leading-snug group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
          {article.title}
        </h3>

        <p className="text-sm text-slate-600 dark:text-slate-400 line-clamp-3 mb-4 flex-1 leading-relaxed">
          {article.excerpt}
        </p>
...
export function ProjectCard({ project, onClick }: ProjectCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-1">
        <h3 className="text-lg font-bold tracking-tight text-slate-900 dark:text-slate-50 mb-2 leading-snug group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
          {project.title}
        </h3>

        <p className="text-sm text-slate-650 dark:text-slate-400 line-clamp-2 mb-4 flex-1 leading-relaxed">
          {project.description}
        </p>
        ...
        <div className="bg-slate-50 dark:bg-slate-950 rounded-lg p-3 border border-slate-150 dark:border-slate-850/60 mb-4">
          <p className="text-xs uppercase font-mono text-slate-400 dark:text-slate-500 mb-2 tracking-wider flex items-center gap-1">
          ...
          <div className="grid grid-cols-3 gap-2">
            {project.metrics.slice(0, 3).map((metric, i) => (
              <div key={i} className="text-center leading-none border-r border-slate-200/50 dark:border-slate-800/50 last:border-r-0">
                <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-1">{metric.label}</p>
                <p className="text-xs font-mono font-bold text-slate-800 dark:text-slate-200 mt-1">{metric.value}</p>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="p-4 flex flex-col flex-1">
        <h4 className="text-base font-bold text-slate-900 dark:text-slate-100 mb-2 leading-snug line-clamp-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
          {video.title}
        </h4>
        <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 mb-4 flex-1">
          {video.description}
        </p>
        <div className="flex items-center justify-between text-xs font-mono text-slate-400 pt-3 border-t border-slate-150 dark:border-slate-800">
          <span className="flex items-center gap-1">
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 p-4 text-white flex flex-col justify-end translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
        <span className="self-start text-[10px] uppercase font-mono tracking-widest bg-white/20 border border-white/10 px-2 py-0.5 rounded mb-2">
          {item.category}
        </span>
        <h4 className="text-base font-bold leading-tight select-none">{item.title}</h4>
        <p className="text-sm text-slate-300 mt-1 line-clamp-1 group-hover:line-clamp-none transition-all select-none">
          {item.description}
        </p>
        <div className="flex items-center justify-between text-xs font-mono text-slate-400 pt-2 border-t border-white/10 mt-2 select-none">
  ...
  return (
    ...
    >
      <div className="flex gap-4 items-start mb-4">
        ...
        />
        <div>
          <h4 className="font-bold text-slate-900 dark:text-slate-50 text-lg">{member.name}</h4>
          <p className="text-sm text-blue-600 dark:text-blue-400 font-bold mb-1.5">{member.role}</p>
          <div className="flex flex-wrap gap-1">
            {member.specialty.slice(0, 3).map((spec, index) => (
              <span key={index} className="text-[10px] px-1.5 py-0.5 rounded-sm bg-slate-100 dark:bg-slate-800 text-slate-650 dark:text-slate-300 border border-slate-200/50 dark:border-slate-700/50 font-mono">
                {spec}
              </span>
            ))}
          </div>
      ...
      <p className="text-sm text-slate-600 dark:text-slate-350 leading-relaxed mb-4 flex-1">
        {member.bio}
      </p>
